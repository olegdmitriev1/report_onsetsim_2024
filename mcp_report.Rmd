...
### Editing the 'summary' function to produce median values instead of means
Below is the code for the 'summary' function used in the mcp package to produce the summary table showing the mean estimate for the change point as well as the upper and lower bounds. The code was edited to create summary.mcpfit.median function (to completment the existing summary.mcp function) to give a summary table with median values instead of mean values. 
...
for(S in 1:nsim){
  ...
  simres.pelt <- Xf[res.pelt@cpts[1]]
  ...
  # BKY
  # bky.res <- mutoss::two.stage(perm.pvals, alpha = aath)
...
save(simres.cs, simres.cp, simres.mcp.median, simres.mcp.mean, simres.fdr, simres.pelt,
     file = "./data/onsetsim_all_methods_sim.RData")
...
load("./data/onsetsim_all_methods_sim_cs_cp_fdr.RData")
load("./data/onsetsim_all_methods_sim_mcp.RData")
load("./data/onsetsim_all_methods_sim_pelt.RData")
...
df <- tibble(onsets = c(simres.cp, simres.cs, simres.mcp.median, simres.mcp.mean, simres.fdr, simres.pelt),
             method = factor(c(rep("change point: BinSeg", length(simres.cp)),
                               ...
                               rep("Change point: PELT", length(simres.pelt))))
)
...
### Scatter plot comparing mean and median outputs of mcp 
```{r, warning=FALSE, fig.height=6, fig.width=7}
load("./data/onsetsim_all_methods_sim_mcp.RData")

df <- data.frame(simres.mcp.mean, simres.mcp.median)
ggplot(data = df, aes(x = simres.mcp.median, y = simres.mcp.mean)) +
  geom_point(color = "#009E73", size = 3) +  
  geom_smooth(method = "lm", color = "#000000") +  
  labs(subtitle = "MCP mean vs median onset values",
       x = "Median Values", 
       y = "Mean Values") +
  theme_gar
```
...
#   file = "./data/main_sim_all_methods1.RData")
...
# df <- tibble(onsets = c(simres.cp, simres.cs, simres.mcp, simres.fdr, simres.pelt, simres.ecp),
#              Method = factor(c(rep("BinSeg", length(simres.cp)),
#                                rep("Cluster Sum", length(simres.cs)),
#                                rep("Bayesian (mcp)", length(simres.mcp)),
#                                rep("FDR", length(simres.fdr)),
#                                rep("PELT", length(simres.pelt)),
#                                rep("cp3o_delta", length(simres.ecp))
#                               )))

df <- tibble(onsets = c(simres.cp, simres.pelt),
             Method = factor(c(rep("BinSeg", length(simres.cp)),
                               rep("cp3o_delta", length(simres.pelt))
                              )))
...
print(paste("Non-parametric (ecp) =",round(median(simres.ecp, na.rm=TRUE) - true_onset,0)))
```
...
print(paste("Non-parametric (ecp) =", round(mean(abs(simres.ecp - true_onset), na.rm = TRUE), digits = 1)))
```
...
print(paste("Non-Parametric (ecp) =",round(var(simres.ecp, na.rm = TRUE), digits=0)))
```
...
print(paste("Non-parametric (ecp) =",round(100*mean((simres.ecp - true_onset) < 0, na.rm = TRUE), digits=1),"%"))
```
...
print(paste("BinSeg =",round(100*mean((simres.cp - true_onset) <= -40, na.rm = TRUE), digits=1),"%"))
print(paste("mcp =",round(100*mean((simres.mcp - true_onset) <= -40, na.rm = TRUE), digits=1),"%"))
print(paste("FDR =",round(100*mean((simres.fdr - true_onset) <= -40, na.rm = TRUE), digits=1),"%"))
print(paste("PELT =",round(100*mean((simres.pelt - true_onset) <= -40, na.rm = TRUE), digits=1),"%"))
print(paste("Non-parametric (ecp) =",round(100*mean((simres.ecp - true_onset) <= -40, na.rm = TRUE), digits=1),"%"))
```


### New sim results with adjusted get_earliest_cp function
#### Merging simulations results 
```{r}
...
# Initialize lists to store combined data for each variable
...
# Loop through each file, load data, and combine it
for (file in files) {
  load(file)
...
# Convert the list of matrices into single matrices for each variable
...
save(simres.cs, simres.fdr, simres.cp, simres.pelt, simres.mcp, simres.ecp,
     file = "./big_sim_joined_data.RData")

```

```{r, warning=FALSE, fig.height=6, fig.width=8}
load("./big_sim_data/big_sim_joined_data.RData")
