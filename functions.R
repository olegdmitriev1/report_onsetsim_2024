...
# INPUTS:
# Function needed to extract the earliest change point from the algorithm from the ecp package
# cps = list of change point locations identified by the algorithm 
# Xf  = a vector of time points 
#
# OUTPUT:
# earliest_time = the time of the earliest change point detected by ecp
get_earliest_cp <- function(cps, Xf) {
  # Filter the change points based on the time window
  valid_cps <- cps[Xf[cps] >= 0 & Xf[cps] <= 250]
