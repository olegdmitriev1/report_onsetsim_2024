...
get_summary = function(fit, width, varying = FALSE, prior = FALSE) {
  ...
  estimates = samples %>%
    ...
    tidybayes::median_hdci(.data$value, .width = width) %>%
  ...
  if(!is.null(sim_list)) {
    ...
    estimates = estimates %>%
      ...
      dplyr::select("name", "match", "sim", "median", "lower", "upper")
  }
...
#'
summary.mcpfit.median = function(object, width = 0.95, digits = 2, prior = FALSE, ...) {
  ...
  if (!is.null(samples)) {
    ...
    if (!is.null(fit$pars$varying))
      cat("\nUse `ranef.med(fit)` to summarise the varying effect(s):", paste0(fit$pars$varying, collapse = ", "))
...
#' @export
ranef.med = function(object, width = 0.95, prior = FALSE, ...) {
  assert_ellipsis(...)
...
# Below is code needed to run the 'assert' functions which are needed for the code above. 
