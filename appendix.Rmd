...
Notes
- develop prior section - could run a simulation to help show how prior affects posterrior
- describe how the pelt algorithm works in more detail when it is introduced as a potential alternative
- do citations 

## Bayesian Multiple Change Points (mcp) Detection Algorithm 
### Example time course

To assess the suitability of the Bayesian multiple change points (mcp) algorithm (Lindelov citation) on detecting effect localisation in EEG data, the algorithm was first applied to an example EEG time course. uniform prior of 100-200ms were chosen in this instance as it was thought that a researcher working with real EEG data could estimate the location of the effect within a 100ms time frame. The results are shown in figure 1, where it is evident that the algorithm was able to estimate the effect onset at 178ms (with the true onset being at 160ms). 
...
**Structure figures in a way that looks good. Try and group them into 3 sets, one for each of the chosen prior tests so each group has different panels, one for the time course, one for the posterior distribution etc. Also make sure to talk about how many times the simulation was repeated and maybe plot them as a simulation run 100 times to show what it looks like.**
...
When the default prior is set, the algorithm naturally detects two change points in the data, one around the time of the true onset and the second one around the time of the offset. This is reflected in the trace plots where there are two traces for different change points that are detected. 
...
The penalty multiplier seemingly appears to be an effective method of limiting the sensitivity of the PELT algorithm, as when compared to running the algorithm without the penalty, multiple change points are detected 
The penalty multiplier appears to be an effective method of limiting the sensitivity of the PELT algorithm when compared to running the algorithm without a penalty setting (figure x). 
...
### Onset distributions 
To get a better understanding of the performance of the PELT algorithm when estimating onsets on varying EEG data, a simulation was done with 10,000 iterations for every penalty multiplier value

To understand how varying penalty multiplier values can affect the distribution of onsets across changing EEG data, a simulation was done with 10,000 iterations for every penalty multiplier value. The results, shown in figure x, indicate that a lot of onsets are being severely underestimated, as evidenced by the long tail. In particular, the tail is heavily populated by onsets obtained from using lower penalty values (identified by purple shaded lines). 
...
The results of the simulation ran with the limits imposed on the PELT algorithm, shown in figure x, indicate that the majority of underestimated change points still occur when lower penalty multiplier values are used (as indicated by the blue and purple shaded lines). Multipliers 5, 10, 15, 20, 25 and 30 were isolated and compared in figure x, where it can be seen that lower multiplier values produce more negatively biased. 
