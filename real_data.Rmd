...
p.cs <- ggplot(df, aes(x)) + theme_gar + 
  ...
  labs(x = "Time in ms", y = bquote(t^2)) +
  ggtitle(paste("Cluster based test estimated onsets: session 1 =",cs.onset.p2.s1,"ms, session 2 =",cs.onset.p2.s2,"ms")) + 
...
p.s1.cs <- ggplot(data = df, aes(x = change.points, color = session)) + theme_gar +
  ...
  scale_color_manual(values = categ.palette) +
  labs(x = "Time in ms", y = "Frequency", color = "Session") +
  ggtitle(paste("Cluster test, median onset =",session1.120.median.mcp,"ms")) +
...
cs.data.s1 <- cs.data.s1 %>% mutate(method = "Cluster test")
...
combined_df <- combined_df %>%
  mutate(median_onset = case_when(
    ...
    method == "Cluster test" ~ session1.120.median.cs
  ))
